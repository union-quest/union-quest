...
const config = {
  ...
  kit: {
    adapter: adapter_ipfs({
      assets: outputFolder,
      pages: outputFolder,
      removeSourceMap: pkg.name === 'jolly' + '-roger-web' ? false : true, // source map are exposed for union-quest
      copyBeforeSourceMapRemoval: pkg.name === 'jolly' + '-roger-web' ? undefined : 'release', // source map are exposed for union-quest
